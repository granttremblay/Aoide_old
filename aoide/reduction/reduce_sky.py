...
select_sky = (MUSE_name==name) & (MUSE_type=='SKY')
...
for i in range(len(times)):
    select_TPL = (MUSE_type=='SKY') &  (MUSE_name==name) & (time==times[i])
    ...
    for j in range(len(MUSE_files[select_TPL])):
        tags={'PIXTABLE_OBJECT':[ 'PIXTABLE_OBJECT_%04d-%02d.fits'%(j+1,z+1) for z in range(24)]}
        ...
        muse.create_sof_scipost('scipost_temp.sof',tags,time=time[i].split('T')[0])
        muse.compute_scipost('scipost_temp.sof',cores,save='individual,skymodel',skymethod='simple',darcheck='none',skymodel_frac=0.9,astrometry='TRUE')
        if muse.nocache:
            os.system('nocache cp SKY_SPECTRUM_0001.fits %s/%s/SKY_SPECTRUM_%s_%02d.fits'%(dir_out,name.replace(' ',''),times[i],expno[j])) 
        else:
            os.system('cp SKY_SPECTRUM_0001.fits %s/%s/SKY_SPECTRUM_%s_%02d.fits'%(dir_out,name.replace(' ',''),times[i],expno[j]))
        if muse.nocache:
            os.system('nocache cp PIXTABLE_REDUCED_0001.fits %s/%s/PIXTABLE_REDUCED_%s_%02d.fits'%(dir_out,name.replace(' ',''),times[i],expno[j])) 
        else:
            os.system('cp PIXTABLE_REDUCED_0001.fits %s/%s/PIXTABLE_REDUCED_%s_%02d.fits'%(dir_out,name.replace(' ',''),times[i],expno[j]))
#        break
        os.system('rm PIXTABLE_REDUCED_0001.fits')
        m+=1
    os.chdir(cwd)
